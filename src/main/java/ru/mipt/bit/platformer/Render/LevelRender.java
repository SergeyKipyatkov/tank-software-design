...
public class LevelRender {
    ...
    public void setObstacle(IObstacleInLevel levelObstacle){
        TiledMapTileLayer groundLayer = getSingleLayer(this.tiledMap);

        for (GameObjectEntity entity : levelObstacle.getObstacleEntities()) {
            moveRectangleAtTileCenter(groundLayer, entity.texture.getRectangle(), entity.conversion.getPosition());
        }
    }
